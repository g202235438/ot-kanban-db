...
-- The 19 card URLs and Status values come from data/raw/cones-project-view1.tsv.
-- Issue metadata is joined from data/raw/cones-issues.json.
begin;
...
select p.project_id,s.status_id,m.milestone_id,u.user_id,v.issue_number,null,v.issue_url,v.title,v.body,v.issue_state,v.created_at::timestamptz,v.closed_at::timestamptz,v.status_position,v.card_position from public.projects p cross join (values
...
|---|---|---|
| **HYUN JIZEL** | `DONE` | `3` |
', 'closed', '2026-09-12T16:20:33Z', '2026-09-12T16:20:58Z', 'seune-h0203', null)) v(issue_number,status_name,status_position,card_position,issue_url,title,body,issue_state,created_at,closed_at,author_login,milestone_title) join public.statuses s on s.project_id=p.project_id and s.status_name=v.status_name left join public.milestones m on m.project_id=p.project_id and m.title=v.milestone_title join public.users u on u.github_username=v.author_login where p.github_repo_full_name='seune-h0203/cones';
