...
create table if not exists public.issue_details (
  task_id bigint primary key references public.tasks(task_id) on delete cascade,
...
create table if not exists public.issue_timeline_events (
  ...
  verification_status text not null default 'unconfirmed'
    check (verification_status in ('verified', 'unconfirmed')),
  note text
);
...
create table if not exists public.project_status_history (
  ...
  task_id bigint references public.tasks(task_id) on delete cascade,
  source_event_id bigint unique references public.issue_timeline_events(event_id)
    on delete cascade,
  occurred_at timestamptz not null,
  actor_login text,
